...
class ImapServer:
    ...
    def is_malicious(self, file_to_scan):
        """Scan the file with virus total
    ...
    def delete_mailbox(self):
        try:
            ...
            time.sleep(1)
            for email_id in tqdm(reversed(id_list), desc='\tDeleting your emails'.expandtabs(15), total=len(id_list)):
                mail.store(email_id, '+FLAGS', '\\Deleted')
                mail.expunge()

            time.sleep(1)
            print('\tdeleting your emails Succeed ...'.expandtabs(15))
            mail.logout()
        except Exception as e:
    ...
    def read_mailbox(self):
        try:
            ...
            time.sleep(1)
            if len(id_list) == 0:
                print('\tYour mailbox is empty'.expandtabs(15))
                return
